...
var router = express.Router(); //change the below urls of router.get to /admin/given_value later
...
router.get('/', authController.isLoggedIn, function (req, res) {
  //This means that whenever we go to "/" page in url, do this function (takes a request and response obj)
  res.render('index', {
...
router.get('/requests', userController.isLoggedIn, userController.viewRequests); //You can create a get and post request for same url also

router.post('/visitors', userController.find); //Add user static form page from visitor page button

router.get('/addvisitor', userController.isLoggedIn, function (req, res) {
...
router.get('/editvisitor/:id', userController.isLoggedIn, userController.editVisitor); //Send data from add user page form

router.post('/addvisitor', userController.addVisitor); //Send data from edit user page form

router.post('/editvisitor/:id', userController.updateVisitor); //Get visitors page after Delete data

router.get('/deletevisitor/:id', userController.isLoggedIn, userController.deleteVisitor); //Get requests page after status change

router.get('/approvevisitor/:id', userController.isLoggedIn, userController.approveVisitor);
router.get('/rejectvisitor/:id', userController.isLoggedIn, userController.rejectVisitor); //View the visitor's visitor Pass

router.get('/viewvisitor/:id', userController.isLoggedIn, userController.viewVisitor);
module.exports = router; //Module exports are the instructions that tell Node. js which bits of code (functions, objects, etc.) to export from a
