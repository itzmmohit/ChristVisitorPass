...
db.connect((error) => { //we give a parameter called error, so that if an error is coming up, deal with it somehow
    if(error){
        ...
        console.log("MySQL (FAC) Connected...")
    }
})
...
exports.register = (req, res) => {
    ...
    const {name, email, password, passwordConfirm, facultyCode} = req.body;
    ...
    db.query('SELECT email FROM guests WHERE email = ?', [email], async(error, results) => {
        ...
        //The below query is similiar to: INSERT INTO guests SET nameAttributeInTable=nameValueFromForm, and so on...
        //Then we run a callback fn (a function done after a function).
        db.query('INSERT INTO guests SET ? ', {name: name, email: email, password: hashedPassword}, (errors, results) => {
            if(error)
            {
                console.log(error);
            }
...
exports.login = async(req,res) => {
    ...
    try {
        ...
        db.query('SELECT * FROM guests WHERE email = ?', [email], async(error, results) => {
            console.log(results)
            ...
            if(!results.length  || !(await bcrypt.compare(password,results[0].password))) //If there are no guests with that email || the password from form and db don't equal.
            {
                //status code of 401 is forbidden code for not being able to login properly.
                res.status(401).render('flogin', {
            ...
            else
            {
                ...
                //create cookie
                res.cookie('fCookie', token, cookieOptions);
