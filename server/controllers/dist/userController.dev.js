...
var _require = require('util'),
    promisify = _require.promisify; // To add validation on back-end: use the module Express-validator
...
exports.view = function (req, res) {
  ...
  db.query('SELECT * FROM visit', function (err, rows) {
    if (!err) {
      //NOTE: if not error
...
exports.viewRequests = function (req, res) {
  db.query('SELECT * FROM visit WHERE status = ?', ['Pending'], function (err, rows) {
    if (!err) {
...
exports.find = function (req, res) {
  ...
  db.query('SELECT * FROM visit WHERE name LIKE ? OR purpose LIKE ? OR status LIKE ?', ['%' + searchTerm + '%', '%' + searchTerm + '%', '%' + searchTerm + '%'], function (err, rows) {
    if (!err) {
      res.render('visitors', {
        rows: rows
      });
...
exports.addVisitor = function (req, res) {
  ...
  db.query('INSERT INTO visit SET name = ?, phone = ?, email = ?, purpose = ?, location = ?, status = ?', [vname, vphone, vemail, vpurpose, vlocation, vstatus], function (err) {
    if (!err) {
...
exports.editVisitor = function (req, res) {
  db.query('SELECT * FROM visit WHERE visit_id = ?', [req.params.id], function (err, rows) {
    if (!err) {
...
exports.updateVisitor = function (req, res) {
  var _req$body2 = req.body,
      ...
      vlocation = _req$body2.vlocation,
      vstatus = _req$body2.vstatus;
  console.log(vname);
  db.query('UPDATE visit SET name = ?, phone = ?, email = ?, purpose = ?, location = ?, status = ? WHERE visit_id = ?', [vname, vphone, vemail, vpurpose, vlocation, vstatus, req.params.id], function (err) {
    if (!err) {
      db.query('SELECT * FROM visit WHERE visit_id = ?', [req.params.id], function (err, rows) {
        if (!err) {
          res.render('editvisitor', {
...
exports.deleteVisitor = function (req, res) {
  db.query('DELETE FROM visit WHERE visit_id = ?', [req.params.id], function (err) {
    if (!err) {
...
exports.approveVisitor = function (req, res) {
  db.query('UPDATE visit SET status = ? WHERE visit_id = ?', ['Approved', req.params.id], function (err) {
    if (!err) {
...
exports.rejectVisitor = function (req, res) {
  db.query('UPDATE visit SET status = ? WHERE visit_id = ?', ['Rejected', req.params.id], function (err) {
    if (!err) {
      ...
      db.query('SELECT labels.label_name, COALESCE(visit_counts.visitor_count, 0) AS visitor_count FROM (SELECT "ALUMNI" AS label_name UNION SELECT "ADMISSION" AS label_name UNION SELECT "BANK" AS label_name UNION SELECT "FEST" AS label_name UNION SELECT "GUEST" AS label_name UNION SELECT "MEET" AS label_name UNION SELECT "OTHERS" AS label_name) AS labels LEFT JOIN (SELECT UPPER(purpose) AS purpose_name, COUNT(*) AS visitor_count FROM visit WHERE status = "Pending" GROUP BY UPPER(purpose)) AS visit_counts ON labels.label_name = visit_counts.purpose_name ORDER BY labels.label_name', function (err, rows1) {
        if (!err) {
          db.query('SELECT labels.label_name, COALESCE(visit_counts.visitor_count, 0) AS visitor_count FROM (SELECT "ALUMNI" AS label_name UNION SELECT "ADMISSION" AS label_name UNION SELECT "BANK" AS label_name UNION SELECT "FEST" AS label_name UNION SELECT "GUEST" AS label_name UNION SELECT "MEET" AS label_name UNION SELECT "OTHERS" AS label_name) AS labels LEFT JOIN (SELECT UPPER(purpose) AS purpose_name, COUNT(*) AS visitor_count FROM visit WHERE status = "Rejected" GROUP BY UPPER(purpose)) AS visit_counts ON labels.label_name = visit_counts.purpose_name ORDER BY labels.label_name', function (err, rows2) {
            if (!err) {
              db.query('SELECT status as label_name, COUNT(*) AS visitor_count FROM visit GROUP BY status', function (err, rows3) {
                if (!err) {
                  db.query('SELECT location as label_name, COUNT(*) AS visitor_count FROM visit GROUP BY location', function (err, rows4) {
                    if (!err) {
